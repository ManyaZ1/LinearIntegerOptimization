...
class SatelliteScheduler:
    ...
    def build_milp_model(self):
        ...
        for satellite in self.satellites:
            for w_idx, (tw_start, tw_end) in enumerate(time_windows):
    ...
    def _create_time_windows(self, window_size_hours: int = 4) -> List[Tuple[datetime, datetime]]:
        """Create time windows for power constraint"""
    ...
    def visualize_schedule(self, figname="enhanced_satellite_schedule.png",figsize=(15, 12)):
        ...
        for satellite in self.satellites:
            ...
            for i, obs in enumerate(sat_obs_sorted):
                ...
                start_hours = (obs.start_time - self.observations[0].start_time.replace(hour=0, minute=0, second=0, microsecond=0)).total_seconds() / 3600
        ...
        ax1.set_xlim(0, self.time_horizon+1)
...
def extract_scenario_from_json(json_path: str, output_dir: str = "results"):
    ...
    #time_horizon = scenario.get("time_horizon", 24)  # default to 24 hours if not specified
    return satellites, targets, scenario["name"],time_horizon 
...
def scenario_picker(x :int):
    if x == "1":
        ...
        return satellites, targets, "example_scenario", 24  # default time horizon for example scenario
    elif x == "2":
        cur_dir= os.path.dirname(os.path.abspath(__file__))
        ...
        satellites, targets, name,time_horizon =extract_scenario_from_json(json_path)
        return satellites, targets,name, time_horizon  # default time horizon for JSON scenarios
    elif x == "3":
        ...
        return satellites, targets, "custom"+in_name, time_horizon  # return custom scenario name and time horizon
    else:
...
def main():
    ...
    satellites, targets, scenarioname, time_horizon = scenario_picker(x)
    ...
    scheduler = SatelliteScheduler(satellites, targets, time_horizon=time_horizon, use_conflict_degree=use_conflict_degree)
